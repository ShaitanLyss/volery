 ...
 *
 *  `Hit` gained it when the widget's own search started drawing these rows, and
 *  it puts a `serde` path into a lift that deliberately links nothing but
 *  `serde_json` — the whole point of which is that it still runs when the
 *  dependency graph does not, which for a Spotify file is not hypothetical.
 *
 *  Safe because a `Serialize` impl cannot change what any assertion here
 *  observes: the tests compare `Hit`s with `PartialEq` and read their fields.
 *  Note the narrowness on purpose — `build.md` warns that a lift dropping
 *  `#[derive(PartialEq)]` would compile into a different thing and only say so
 *  at the assertion, so this removes one named derive rather than tidying
 *  attributes in general. */
function withoutSerialize(item: string): string {
  return item.replace(/,\s*serde::Serialize/g, "");
}

const body = [
  "//! GENERATED by tools/lift-selector.ts — do not edit, do not keep.",
  "use serde_json::{json, Value};",
  ...ITEMS.map(find).map(withoutSerialize),
  "#[cfg(test)]",
  "mod tests {",
  "    use super::*;",
  ...TESTS.map(findTest),
