...
/** The browser tool prefix, which `append_prompt` prints and does not own.
 *
 *  It lives in `hooks.rs` because the hook that routes on it is the half that
 *  breaks silently if the two drift — so the paragraph borrows it rather than
 *  spelling it twice, and this lift borrows it for the same reason a stub would
 *  be worthless: asserting that the prompt says `mcp__browser__` against a copy
 *  this file invented would prove nothing about the name a card is actually
 *  given. `tools/lift-browser.ts` holds the other end, that the constant
 *  matches the server `ask::mcp_config` registers. */
const FROM_HOOKS = ["const BROWSER_PREFIX"];
...
const body = [
  ...
  `pub mod hooks {\n${FROM_HOOKS.map((w) => hooks.find(w)).join("\n\n")}\n}`,
  /* The same `use` `supervisor.rs` has, so `append_prompt`'s format string
     resolves the bare name exactly as it does in the crate. */
