...
const ITEMS = [
  ...
  "fn may_close",
  "fn close_question",
  /* The brief, and the bound that is off. `clip_brief` has no caller at all
     (`MAX_PROMPT` is `None`), which is exactly why it is lifted: the day a cap
     comes back is the day it has to already be boundary-aware and already
     announce itself at both ends, and nothing else would notice if it stopped
     being either. */
  "const MAX_LIVE",
  "const MAX_PER_HOUR",
  "const ONE_GENERATION",
  "const MAX_PROMPT",
...
const TESTS = [
  ...
  /* And the brief's three: the cap is off, the description says so, and the
     clipping that is parked behind it is correct in advance. */
...
const body = [
  ...
  /* `clip_brief`'s marker names the tool to reach the parent with, so it wants
     the one constant that says how a card spells a tool name. Lifted rather
     than written out here for the reason `handle_of` is: a copy that said
     `mcp__volery__` would pass a test about nothing. */
  `pub mod supervisor {\n${
    readFileSync("src-tauri/src/supervisor.rs", "utf8")
      .split(/\r?\n/)
      .find((l) => /^\s*(pub(\([a-z]+\))?\s+)?const MCP_PREFIX\b/.test(l)) ??
      (() => {
        throw new Error("MCP_PREFIX is no longer in supervisor.rs — has it moved?");
      })()
  }\n}`,
  ...ITEMS.map(find),
  "#[cfg(test)]",
