...
<div class="catch" onpointerdown={onclose} oncontextmenu={onclose} role="presentation">
  ...
  >
    {#each items as it, i (i)}
      ...
      {:else}
        <!-- Both halves of the merge below, and each is load-bearing on its
             own. `onpointerenter` closes an open submenu when the pointer
             crosses a plain row, so the nested list does not hang over the menu
             you have moved on to. The modifier on `onclick` is the preset
             menu's second gesture, which `ContextMenu` has no room to give a
             row of its own. -->
        <button
          class="row"
